...
async function submitAddCustomersForm() {
    ...
    .then(data => {
        ...
        if (data.data.message == `Duplicate entry '${email}' for key 'email'`) {
            alert("Sorry!! This email is already being used by another user.")
        } else if (data.data.message == `Duplicate entry '${contact}' for key 'contact'`) {
            alert("Sorry!! This email is already being used by another user.")
        } else {
            setTimeout(function() {                  
                if (data.data.success == true) {
...
async function displayCustomers() {
    ...
    await fetch(baseurl+`/auth/customers`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function searchCustomer() {
    ...
    await fetch(baseurl+`/auth/users/any/admin`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({ anysearch })
    })
    .then(response => response.json())
    .then(item => {
        console.log(item);
...
async function deleteMyProductPopUp() {
    ...
    .then(item => {
        ...
        if (item.data.success == true) {
            ...
            window.location.href = 'admin.html#content3';
...
async function UpdateCustomerInfo() {
    ...
    .then(item => {
        // console.log(item);
        if (item.data.message == `Duplicate entry '${email}' for key 'email'`) {
            alert("Sorry!! This email is already being used by another user.")
        } else if (item.data.message == `Duplicate entry '${contact}' for key 'contact'`) {
            alert("Sorry!! Please review the Contant number again. you might have made a mistake")
        } else {
            if (item.data.success == true) {
